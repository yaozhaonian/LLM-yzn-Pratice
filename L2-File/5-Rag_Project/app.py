...
async def upload_files(files: List[UploadFile] = File(...), knowledge_name: str = Form(...)):
    ...
    for file in files:
        ...
        # 🔴 新增：验证文件内容（防止扩展名与实际格式不符）
        if ext == ".docx":
            # 检查文件头是否为 ZIP 格式（docx 本质是 ZIP）
...
@app.post("/api/chat")
async def chat(query: str = Form(...), knowledge_name: str = Form(...)):
    """聊天问答"""
    ...
    result = rag_engine.generate_answer(query, n_results=12)
