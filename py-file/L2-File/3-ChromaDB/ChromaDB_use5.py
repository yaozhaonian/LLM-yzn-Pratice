...
# ======================
# 3. 向量库管理类 (优化版: 使用 PersistentClient)
# ======================
class VectorStoreManager:
    ...
    def __init__(self, vector_store_dir: Path, embedding, logger: logging.Logger):
        ...
        self.collection_name = "rag_collection" # 定义固定的集合名称
        self.vector_store: Optional[Chroma] = None
    
    def _get_client(self) -> chromadb.PersistentClient:
        """获取或创建 PersistentClient"""
        if self.client is None:
            self.logger.info(f"初始化 PersistentClient: {self.vector_store_dir}")
    ...
    def close(self):
        ...
        if self.client is not None:
            try:
                self.client.close()
                self.logger.info("ChromaDB 客户端已关闭，文件锁已释放")
            except Exception as e:
                self.logger.warning(f"关闭客户端时出现警告: {e}")
            finally:
                self.client = None
    ...
    def load_existing(self) -> bool:
        ...
        try:
            ...
            if not any(c.name == self.collection_name for c in collections):
                self.logger.warning(f"集合 '{self.collection_name}' 不存在，视为空库")
                return False

            # 使用 LangChain 包装现有的 Client 和 Collection
            ...
            print(f"✓ 向量库加载完成: {self.vector_store_dir} (Docs: {count})")
            return True
    ...
    def create_from_documents(self, documents, text_splitter) -> int:
        ...
        if self.vector_store_dir.exists():
            ...
            try:
                ...
                self.logger.info("旧向量库目录已清理")
            except PermissionError as e:
                self.logger.error(f"无法删除旧目录，请检查是否有其他程序占用: {e}")
                raise e

        # 重新创建客户端（因为目录被删了，旧客户端实例已无效）
        self.client = None 
        client = self._get_client()
        ...
        count = self.vector_store._collection.count()
        self.logger.info(f"向量库创建完成: {count} 个片段")
